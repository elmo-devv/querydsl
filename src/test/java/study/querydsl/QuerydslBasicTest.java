...
//                .limit(1).fetchOne()
    ...
    /**
     * 회원과 팀을 조인하면서 팀 이름이 teamA인 팀만 조인, 회원은 모두 조회 JPQL : select m, t from Member m left join m.team t on tmname =
     * "teamA"
     */
    @Test
    ...
    /**
     * 연관관계가 없는 엔티티를 외부 조인 회원의 이름이 팀 이름과 같은 대상 외부 조인
     */
    @Test
    ...
    public void fetchJoinUse() {
        ...
        Member member = jpaQueryFactory
                .selectFrom(QMember.member)
                .join(QMember.member.team).fetchJoin()
                .where(QMember.member.username.eq("member1"))
                .fetchOne();
        boolean loaded = emf.getPersistenceUnitUtil().isLoaded(member.getTeam());
